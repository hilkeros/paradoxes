...
vec4 advect(vec2 ab, vec2 vUv, vec2 step, float sc) {
    ...
    vec4 uv =    texture2D(tex0, fract(aUv));
    vec4 uv_n =  texture2D(tex0, fract(aUv+n));
    vec4 uv_e =  texture2D(tex0, fract(aUv+e));
    vec4 uv_s =  texture2D(tex0, fract(aUv+s));
    vec4 uv_w =  texture2D(tex0, fract(aUv+w));
    vec4 uv_nw = texture2D(tex0, fract(aUv+nw));
    vec4 uv_sw = texture2D(tex0, fract(aUv+sw));
    vec4 uv_ne = texture2D(tex0, fract(aUv+ne));
    vec4 uv_se = texture2D(tex0, fract(aUv+se));
...
void main(){
    ...
    // vec2 vUv = vTexCoord.xy / iResolution.xy;
    ...
    vec4 uv =    texture2D(tex0, fract(vUv));
    vec4 uv_n =  texture2D(tex0, fract(vUv+n));
    vec4 uv_e =  texture2D(tex0, fract(vUv+e));
    vec4 uv_s =  texture2D(tex0, fract(vUv+s));
    vec4 uv_w =  texture2D(tex0, fract(vUv+w));
    vec4 uv_nw = texture2D(tex0, fract(vUv+nw));
    vec4 uv_sw = texture2D(tex0, fract(vUv+sw));
    vec4 uv_ne = texture2D(tex0, fract(vUv+ne));
    vec4 uv_se = texture2D(tex0, fract(vUv+se));
